...
const getBinaryMarkets = createServerFn({ method: 'GET' }).handler(async () => {
  try {
    return await fetchBinaryMarketsFromCache('kalshi', getCacheServerUrl())
  } catch {
    return null
  }
})
...
const getDutchingEvents = createServerFn({ method: 'GET' }).handler(async () => {
  try {
    return await fetchDutchingEventsFromCache('kalshi', getCacheServerUrl())
  } catch {
    return null
  }
})
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-zinc-600">// CFTC-regulated • centralized exchange</p>
      </div>
      ...
      <div className="border border-zinc-800 bg-zinc-900/30 p-8">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-600">connecting to kalshi cache</p>
          </div>
        </div>
      </div>
    </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-zinc-600">
                  kalshi data is not yet cached. ensure the collector server has KALSHI_KEY_ID and KALSHI_PRIVATE_KEY configured.
                </p>
              </div>
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-green-500">$</span>
            <span className="text-zinc-100 text-sm sm:text-base">scan --market kalshi --mode {activeTab}</span>
          </div>
          <p className="text-sm text-zinc-600 hidden sm:block">// CFTC-regulated • centralized exchange</p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <td className="max-w-[240px] truncate px-3 py-2 market-title">
                    <a
                      href={`https://kalshi.com/events/${event.id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-zinc-300 hover:text-green-400 hover:underline"
                    >
